...
class CorrConvFunction(Function):
	...
	def forward(ctx, input, weight, bias=None, stride=1, padding=0, lamda=0.005):
		# ipdb.set_trace()
		ctx.save_for_backward(input, weight, bias)   #保存至backward
		ctx.lamda = lamda
		ctx.stride = stride
		ctx.padding = padding
	...
	def backward(ctx, grad_output):
		...
		X1 = Variable(input1.data.clone(), requires_grad=True)  #clone()进行深拷贝
		...
		Offset1 = Y1 - torch.mean(Y1, 0, keepdim=True).expand_as(Y1)  #扩展张量中某维数据的尺寸，扩展至跟Y1一样的尺寸
		Offset2 = Y2 - torch.mean(Y2, 0, keepdim=True).expand_as(Y2)
		CrossVar = torch.sum(Offset1 * Offset2, 0, keepdim=True) #相乘求和
		AbsVar1 = torch.sum(Offset1, 0, keepdim=True)  #torch.sum(input,dim,out) dim=0 对列求和；dim=1 对行求和;out一般不指定
		...
		tmpExp_I = torch.pow(Sigma1*Sigma2+eps, -0.5)  #pow(x,y) 返回x的y次方  公式(3)的分子
		...
		Y1.backward(dCorrdY1) #backward()反向传播计算
